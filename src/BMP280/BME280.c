...
static void I2Cx_WriteData(uint16_t Addr, uint8_t Reg, uint8_t Value)
{
	...
	err = i2cm_Start(I2C1, Addr, 0, temiout);
	if (!err)
  {	 
	  buf[0]= Reg;		
	  buf[1]= Value;
	  i2cm_WriteBuff(I2C1, buf, 2, temiout);//AndStop
...
static uint8_t I2Cx_ReadData(uint16_t Addr, uint8_t Reg)
{
  ...
  {	
	  buf[0]= Reg;
	  i2cm_WriteBuff(I2C1, buf, 1, temiout);//AndStop
	  //i2cm_Stop(I2C1, temiout);		
	}		
	else err = 1;
	
	err = i2cm_Start(I2C1, Addr, 1, temiout);
	if (!err)
  {	 
	  i2cm_ReadBuffAndStop(I2C1, buf, 1, temiout);
	}
	else err = 2;	
...
{
	uint32_t value32=BME280_ReadReg(Reg+1);
	value32=value32<<8;
	
	value32|=BME280_ReadReg(Reg);
...
static void I2Cx_ReadData24(uint16_t Addr, uint8_t Reg, uint32_t *Value)
{
	...
	value32=BME280_ReadReg(Reg+2);
	value32=value32<<8;
	
	value32|=BME280_ReadReg(Reg+1);
	value32=value32<<8;	
...
float BME280_ReadPressure(void)
{
  float press_float = 0.0f;
	...
	int64_t val1, val2, p;
	tempBME280=BME280_ReadTemperature(); // must be done first to get t_fine
	BME280_ReadReg_BE_U24(BME280_REGISTER_PRESSUREDATA,&press_raw);
	press_raw =press_raw>> 4;
...
//	sprintf(str1, "\r\n\r\nID: 0x%02X\r\n", value);
//	HAL_UART_Transmit(&huart2,(uint8_t*)str1,strlen(str1),0x1000);
	if((value !=BMP280_ID) && (value !=BME280_ID) )
	{
		Error(10);
		return;
	}
...
//	if	(value32 & BME280_OSRS_T_MSK) {
//			Error(20);
//	}
//	if	(value32 & BME280_OSRS_P_MSK) {
//			Error(30);
//	}
//	if	((value32 >> 8) & BME280_OSRS_H_MSK) {
//			Error(40);
//	}		
//	HAL_UART_Transmit(&huart2,(uint8_t*)str1,strlen(str1),0x1000);
